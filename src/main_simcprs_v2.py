# -*- coding: utf-8 -*-
## This version is differrent with main_simcprs:
# Main_simcprs: encoding entire and then separating batch
# Main_simcprs_v1: Separate bacth before encoding
import os
...
class Model_Classifier_NoAim(nn.Module):
    def __init__(self, base_model, num_classes):
        super(Model_Classifier_NoAim, self).__init__()
        self.base_model = base_model
        self.linear1_1 = nn.Linear(768, 512)
        self.act1_1 = nn.ReLU()
        self.drop1_1 = nn.Dropout(0.1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", type=str, default="./QLoRAPSR/", help="data path")
    ...
    parser.add_argument("--features", type=str, default='TAK', help="Which features")
    ...
    parser.add_argument("--use_category", action="store_true", help="Whether to use category embeddings")
    parser.add_argument("--saved_folder", type=str,default='SimCPRS', help="Save Folder")
    args = parser.parse_args()

    if args.use_aim and args.use_category:
        features_name = args.features + 'SC'
    elif args.use_aim:
    ...
    if args.checkpoint_path:
        is_use_cl_model = "use_finetuned_CL_model"
    else:
        is_use_cl_model = "use_base_model"
    ...
    print(f"use_category: {args.use_category}")
    print(f"Features: {features_name}")
    print(f"Used: {is_use_cl_model}")
    ...
    if args.features == 'TAK':
        X_train = ( data_train['Title'] + " " + data_train['Abstract'] + " " + data_train['Keywords'] ).tolist()
        X_valid = ( data_validate['Title'] + " " + data_validate['Abstract'] + " " + data_validate['Keywords'] ).tolist()
        X_test = ( data_test['Title'] + " " + data_test['Abstract'] + " " + data_test['Keywords'] ).tolist()
    elif args.features == 'TA':
        X_train = ( data_train['Title'] + " " + data_train['Abstract'] ).tolist()
        X_valid = ( data_validate['Title'] + " " + data_validate['Abstract'] ).tolist()
        X_test = ( data_test['Title'] + " " + data_test['Abstract'] ).tolist()
    elif args.features == 'TK':
        X_train = ( data_train['Title'] + " " + data_train['Keywords'] ).tolist()
        X_valid = ( data_validate['Title'] + " " + data_validate['Keywords'] ).tolist()
        X_test = ( data_test['Title'] + " " + data_test['Keywords'] ).tolist()
    elif args.features == 'AK':
        X_train = ( data_train['Abstract'] + " " + data_train['Keywords'] ).tolist()
        X_valid = ( data_validate['Abstract'] + " " + data_validate['Keywords'] ).tolist()
        X_test = ( data_test['Abstract'] + " " + data_test['Keywords'] ).tolist()
    elif args.features == 'T':
        X_train = ( data_train['Title'] ).tolist()
        X_valid = ( data_validate['Title'] ).tolist()
        X_test = ( data_test['Title'] ).tolist()
    elif args.features == 'A':
        X_train = ( data_train['Abstract'] ).tolist()
        X_valid = ( data_validate['Abstract'] ).tolist()
        X_test = ( data_test['Abstract'] ).tolist()
    else:
        X_train = ( data_train['Keywords'] ).tolist()
        X_valid = ( data_validate['Keywords'] ).tolist()
        X_test = ( data_test['Keywords'] ).tolist()
    ...
    class Dataset(torch.utils.data.Dataset):
        def __init__(self, texts, labels):
            self.texts = texts  # Lưu text gốc
            self.labels = labels  # Lưu nhãn

        def __getitem__(self, idx):
            return self.texts[idx], self.labels[idx]  # Trả về text thô, chưa encode

        def __len__(self):
            return len(self.labels)

    def collate_fn(batch, tokenizer, max_len):
        texts, labels = zip(*batch)  # Tách danh sách text & label

        encodings = tokenizer(
            list(texts),  # Encode toàn bộ batch cùng lúc
            truncation=True,
            padding="max_length",
            max_length=args.max_len,
            return_tensors="pt"
        ...
        x = {key: val for key, val in encodings.items()}  # Tạo batch tensor
        y = torch.tensor(labels, dtype=torch.long)  # Chuyển labels thành tensor

        return x, y


    train_dataset = Dataset(X_train, Y_train)
    valid_dataset = Dataset(X_valid, Y_validate)
    ...
    if args.checkpoint_path:   # # Fine-tuned LM checkpoint (by contrastive learning)
        print(f"Loading checkpoint from: {args.checkpoint_path}")
        ...
        print("No checkpoint path provided. Starting with randomly initialized base model.")
    ...
    #
    # # Chuyển numpy array thành tensor theo thiết bị đúng cách
    # aims_embeddings = torch.from_numpy(np.array(aims_embeddings)).float().to(device)
    ...
    def get_latest_checkpoint(checkpoint_dir):
        checkpoint_sort = [f for f in os.listdir(checkpoint_dir) if f.endswith(".pth")]
    ...
    if os.path.exists(result_file_path):
        ...
        with open(result_file_path, "w") as f:
            ...
            f.write(f"Is_use_cl_model: {is_use_cl_model}\n")
    ...
    for epoch in range(epoch, max_epochs):
        ...
        for features, labels in train_loop:
            ...
            if args.use_aim: # If use_aim is True
                logits = model(features, aims_embeddings) # Pass both features and aims embeddings
            else: # If use_aim is False
                logits = model(features) # Pass only features
        ...
        with torch.no_grad():
            ...
            if min_valid_loss > valid_loss:
                ...
                checkpoint_save_path = os.path.join(checkpoint_dir, f"Epoch_{epoch:02d}_{args.saved_folder}_{safe_model_name}.pth")
                trigger_times = 0
                # Saving State Dict
                torch.save(
                    {
